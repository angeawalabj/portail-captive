...
def test_get_current_user_with_token():
    ...
    user = User(username="alice", password_hash=auth.get_password_hash("pass"), role="admin", is_active=True)
    db.add(user)
    db.commit()
    db.refresh(user)
...
def test_role_checks():
    db = next(override_get_db())
    admin_user = User(username="admin1", password_hash=auth.get_password_hash("pass"), role="admin", is_active=True)
    staff_user = User(username="staff1", password_hash=auth.get_password_hash("pass"), role="staff", is_active=True)
